...
const command = {
    ...
    data: new discord_js_1.SlashCommandBuilder().addUserOption(option => option.setName("user")
        .setDescription((0, langtools_1.getMessage)("cmd_bite_option_user_desc", types_1.localization.en))
    ...
    callback: async (client, interaction, loc) => {
        ...
        let emb = new discord_js_1.EmbedBuilder().setTimestamp().setFooter({
            ...
            .setTitle((0, langtools_1.getMessage)("cmd_bite_result_bitten_name", loc)).setDescription((0, langtools_1.parseMessage)("cmd_bite_result_bitten_desc", loc, { firstUserID: interaction.user.id, secondUserID: mentionedUser.id }));
        const fromApi = await axios_1.default.get('https://api.otakugifs.xyz/gif?reaction=bite&format=gif');
