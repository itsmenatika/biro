...
const command: cmdData = {
    ...
    data: new SlashCommandBuilder().addUserOption(option => 
        option.setName("user")
        .setDescription(getMessage("cmd_hug_option_user_desc", localization.en))
        .setNameLocalizations(getFullDictOf("cmd_hug_option_user_name"))
        .setDescriptionLocalizations(getFullDictOf("cmd_hug_option_user_name"))
        .setRequired(true)
    ),
    callback: async (client: Client, interaction: CommandInteraction, loc: localization, connection: Pool) => {
        ...
        .setTitle(getMessage("cmd_hug_result_hugged_name", loc)).setDescription(
                                                            parseMessage("cmd_hug_result_hugged_desc", loc, {firstUserID: interaction.user.id, secondUserID: mentionedUser.id})
                                                            );


        // get image from api (otakugifs)
        const fromApi: AxiosResponse = await axios.get('https://api.otakugifs.xyz/gif?reaction=hug&format=gif');
