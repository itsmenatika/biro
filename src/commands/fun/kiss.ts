...
const command: cmdData = {
    ...
    data: new SlashCommandBuilder().addUserOption(option => 
        option.setName("user")
        .setDescription(getMessage("cmd_kiss_option_user_desc", localization.en))
        .setNameLocalizations(getFullDictOf("cmd_kiss_option_user_name"))
    ...
    callback: async (client: Client, interaction: CommandInteraction, loc: localization) => {
        ...
        .setTitle(getMessage("cmd_kiss_result_kissed_name", loc)).setDescription(
                                                            parseMessage("cmd_kiss_result_kissed_desc", loc, {firstUserID: interaction.user.id, secondUserID: mentionedUser.id})
                                                            );


        // get image from api (otakugifs)
        const fromApi: AxiosResponse = await axios.get('https://api.otakugifs.xyz/gif?reaction=kiss&format=gif');
