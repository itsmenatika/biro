...
const command: cmdData = {
    ...
    data: new SlashCommandBuilder().addUserOption(option => 
        option.setName("user")
        .setDescription(getMessage("cmd_cuddle_option_user_desc", localization.en))
        .setNameLocalizations(getFullDictOf("cmd_cuddle_option_user_name"))
        .setDescriptionLocalizations(getFullDictOf("cmd_cuddle_option_user_name"))
        .setRequired(true)
    ),
    callback: async (client: Client, interaction: CommandInteraction, loc: localization, connection: Pool) => {
        ...
        .setTitle(getMessage("cmd_cuddle_result_cuddled_name", loc)).setDescription(
                                                            parseMessage("cmd_cuddle_result_cuddled_desc", loc, {firstUserID: interaction.user.id, secondUserID: mentionedUser.id})
                                                            );


        // get image from api (otakugifs)
        const fromApi: AxiosResponse = await axios.get('https://api.otakugifs.xyz/gif?reaction=cuddle&format=gif');
