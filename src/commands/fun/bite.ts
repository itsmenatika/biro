...
const command: cmdData = {
    ...
    data: new SlashCommandBuilder().addUserOption(option => 
        option.setName("user")
        .setDescription(getMessage("cmd_bite_option_user_desc", localization.en))
        .setNameLocalizations(getFullDictOf("cmd_bite_option_user_name"))
        .setDescriptionLocalizations(getFullDictOf("cmd_bite_option_user_name"))
        .setRequired(true)
    ),
    callback: async (client: Client, interaction: CommandInteraction, loc: localization) => {
        ...
        .setTitle(getMessage("cmd_bite_result_bitten_name", loc)).setDescription(
                                                            parseMessage("cmd_bite_result_bitten_desc", loc, {firstUserID: interaction.user.id, secondUserID: mentionedUser.id})
                                                            );


        // get image from api (otakugifs)
        const fromApi: AxiosResponse = await axios.get('https://api.otakugifs.xyz/gif?reaction=bite&format=gif');
